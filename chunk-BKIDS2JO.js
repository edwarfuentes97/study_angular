import{ha as r}from"./chunk-G73V3KC4.js";var s=class t{terms=[{term:"Store",definition:`Es el contenedor central del estado global.
...
\u2022 la app creci\xF3 m\xE1s all\xE1 de lo que un solo build tolera

Riesgo:
\u2022 para apps de un solo equipo suele ser overengineering: pagas complejidad operacional sin ganar autonom\xEDa`,topic:"microfrontends"},{term:"Module Federation",definition:`Mecanismo de webpack 5 para cargar c\xF3digo de otro build en runtime.

Conceptos clave:
\u2022 host: la app shell que consume remotes
\u2022 remote: la app que expone m\xF3dulos (exposes)
...
En Angular moderno se usa su evoluci\xF3n: Native Federation (esbuild).`,topic:"microfrontends"},{term:"Native Federation",definition:`Implementaci\xF3n de federaci\xF3n agn\xF3stica del bundler (@angular-architects/native-federation), basada en import maps y esbuild.
...
Ventaja:
\u2022 funciona con el builder esbuild moderno de Angular, sin volver a webpack`,topic:"microfrontends"},{term:"Angular Elements",definition:`Empaqueta un componente Angular como Web Component (custom element) con createCustomElement().

\xDAsalo como frontera framework-agn\xF3stica entre microfrontends:
\u2022 el shell solo conoce la etiqueta HTML (<mi-widget>)
\u2022 el MFE puede migrar de versi\xF3n/framework sin romper el contrato

Riesgo:
\u2022 cada elemento carga su propio runtime de Angular si no compartes dependencias`,topic:"microfrontends"},{term:"Shell",definition:`La aplicaci\xF3n host/contenedor en una arquitectura de microfrontends.

Responsabilidades:
\u2022 routing de alto nivel (qu\xE9 MFE responde a qu\xE9 prefijo de URL)
\u2022 layout global (header, nav)
\u2022 orquestar autenticaci\xF3n y estado compartido m\xEDnimo
\u2022 cargar remotes v\xEDa manifest

Regla mental: el shell debe ser tonto y estable \u2014 cuanta menos l\xF3gica de negocio tenga, mejor.`,topic:"microfrontends"}];topicRoutes={ngrx:{route:"/lab/ngrx",label:"NgRx Lab"},rxjs:{route:"/lab/rxjs",label:"RxJS Lab"},angular:{route:"/lab/angular",label:"Angular Core Lab"},typescript:{route:"/lab/typescript",label:"TypeScript Lab"},architecture:{route:"/lab/architecture",label:"Architecture Lab"},performance:{route:"/lab/performance",label:"Performance Lab"},testing:{route:"/lab/testing",label:"Testing Lab"},playwright:{route:"/lab/playwright",label:"Playwright Lab"},fundamentals:{route:"/lab/fundamentals",label:"Angular Fundamentals"},microfrontends:{route:"/lab/microfrontends",label:"Microfrontends Lab"}};getDefinition(e){return this.terms.find(n=>n.term.toLowerCase()===e.toLowerCase())?.definition??null}search(e){if(!e||e.length<2)return[];let a=e.toLowerCase(),n=[];for(let o of this.terms)if(o.term.toLowerCase().includes(a)||o.definition.toLowerCase().includes(a)){let i=this.topicRoutes[o.topic];n.push({term:o.term,definition:o.definition,topic:i?.label??o.topic,route:i?.route??"/lab/"+o.topic})}return n}getTermsByTopic(e){return this.terms.filter(a=>a.topic===e)}getRoute(e){return this.topicRoutes[e]?.route??"/lab/"+e}static \u0275fac=function(a){return new(a||t)};static \u0275prov=r({token:t,factory:t.\u0275fac,providedIn:"root"})};export{s as a};
